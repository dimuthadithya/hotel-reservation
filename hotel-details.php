...
<body>
  ...
  <section class="hotel-info">
    <div class="container">
      <div class="row">
        <div class="col-lg-8">          <div class="hotel-title">
          ...
          <div class="hotel-description mb-4">
            ...
            <?php endif; ?>
          </div>          <div class="amenities-section mb-4">
          ...
          <div class="rating-summary text-center">
            ...
            <div class="overall-rating mb-2"><?= number_format($hotel['average_rating'], 1) ?></div>
            <div class="rating-text h5 mb-3"><?= getRatingText($hotel['average_rating']) ?></div>
            <?php endif; ?>
            <p class="text-muted">Based on <?= $hotel['total_reviews'] ?> reviews</p>
              <?php            // Get average ratings by category
            $ratingsQuery = "SELECT 
                ROUND(COALESCE(AVG(NULLIF(cleanliness_rating, 0)), 0), 1) as cleanliness,
                ROUND(COALESCE(AVG(NULLIF(service_rating, 0)), 0), 1) as service,
                ROUND(COALESCE(AVG(NULLIF(location_rating, 0)), 0), 1) as location,
                COUNT(*) as total_ratings
                FROM reviews 
                WHERE hotel_id = :hotel_id 
                AND review_status = 'approved'";
            $ratingsStmt = $conn->prepare($ratingsQuery);
            $ratingsStmt->execute(['hotel_id' => $hotel_id]);
            ...
            <div class="rating-bars">
              <div class="rating-bar-item">
                ...
                <div class="rating-bar">
                  <div class="rating-fill" style="width: <?= ($ratings['cleanliness'] * 20) ?>%"></div>
                </div>
                <span><?= $ratings['total_ratings'] > 0 ? number_format($ratings['cleanliness'], 1) : 'No ratings' ?></span>
              </div>
              <div class="rating-bar-item">
                <span>Service</span>
                <div class="rating-bar">
                  <div class="rating-fill" style="width: <?= ($ratings['service'] * 20) ?>%"></div>
                </div>
                <span><?= $ratings['total_ratings'] > 0 ? number_format($ratings['service'], 1) : 'No ratings' ?></span>
              </div>
              <div class="rating-bar-item">
                <span>Location</span>
                <div class="rating-bar">
                  <div class="rating-fill" style="width: <?= ($ratings['location'] * 20) ?>%"></div>
                </div>
                <span><?= $ratings['total_ratings'] > 0 ? number_format($ratings['location'], 1) : 'No ratings' ?></span>
              </div>
            </div>
          </div>
        </div>
